...
\centering
\small
\input{v2_shape_calibration_table.tex}
\caption{V2 shape-calibration stress. Shape gain attenuates asymmetry, onset-lag, and spatial-jump features before rescoring the same cases.}
\label{tab:v2-shape}
\end{table}
...
Table~\ref{tab:v2-shape} stress-tests that reliance. We attenuate the shape features used by the causal rule while leaving residual energy and persistence unchanged. At 0.8 shape gain, causal F1 is still 0.957. At 0.7 shape gain, causal F1 falls to 0.891, below the uncertainty gate's 0.918. At 0.6 shape gain, causal F1 falls to 0.812. This narrows the claim from ``causal residuals solve contact diagnosis'' to ``calibrated residual-shape features can support causal contact diagnosis.''

\section{Limitations}

This is a synthetic diagnostic benchmark. It does not prove robustness on real tactile skins, across object materials, or under human-subject interaction. It also assumes the controller exposes a contact graph and phase label. The v2 stress adds a stronger limitation: if tactile feature extraction under-reports directional asymmetry, onset timing, or spatial jump, a simpler uncertainty gate can outperform the causal residual rule. Those assumptions are exactly what future hardware experiments should stress. The useful artifact here is the falsifiable separation criterion: if calibrated causal residual features do not change the response decision, the mechanism has failed.
